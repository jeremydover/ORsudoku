...
def setLeftRightCondition(self,select1,select2,value):
	for j in range(self.boardWidth-1):
		for i in range(self.boardWidth):
			myCount=self.model.NewIntVar(0,1,'TopDownConditionRow{:d}Col{:d}'.format(i,j))
			countIs1 = self.model.NewBoolVar('TopDownConditionBool')
			...
			self.setConditionalCountLine([(i+1,j+1),(i+1,j+2)],myCount,select1,[['Last']])
			self.setConditionalCountLine([(i+1,j+1),(i+1,j+2)],value,select2,[['Last']],OEI=[countIs1])
			
def setOrthogonalCondition(self,select1,select2,value):
	self.setTopDownCondition(select1,select2,value)
	self.setLeftRightCondition(select1,select2,value)
